...
function consensus(){return safe(()=>window.RMConsensusXI?.state?.(),null)}
...
function consensusSnapshot(){const c=consensus();if(!c||!validXi(c.xi))return null;return {id:'consensus',title:'XI síntesis',xi:cloneXi(c.xi),meta:`${c.total} perspectivas · ${c.unanimous} unánimes · ${c.split} divididos`,capturedAt:new Date().toISOString(),sources:c.sources.map(s=>s.id)}}
...
function renderPrediction(){
  ...
  if(!rec){root.innerHTML='<div class="sa-empty"><b>LIGA DE ESCENARIOS</b><span>Guarda tu XI y congelaremos Proyecto + Señales fuertes + XI síntesis para compararlos después con el once oficial.</span></div>';return}
  const settled=Boolean(rec.scores),hasCommunity=Boolean(rec.scenarios?.community),hasConsensus=Boolean(rec.scenarios?.consensus);
  root.innerHTML=`<div class="sa-head"><div><span>LIGA DE ESCENARIOS</span><h3>${settled?'Qué escenario leyó mejor el XI oficial':'Escenarios congelados antes del XI oficial'}</h3><p>${settled?'Misma regla 0–11 para todos: cuenta si el jugador fue titular, independientemente de la posición.':'Tu XI, Proyecto, Señales fuertes y XI síntesis quedan ligados al guardado. Comunidad solo entra si el consenso ya era final antes del XI oficial.'}</p></div><b>${settled?`Ganador: ${esc(winnerText(rec))}`:`${Object.keys(rec.scenarios||{}).length+1} congelados`}</b></div><div class="sa-scores">${['user','project','strong','consensus','community'].map(id=>scenarioCard(id,rec)).join('')}</div><p class="sa-note">${rec.snapshotLate?'Primera captura creada después de un guardado previo, pero todavía antes del XI oficial. ':''}${hasConsensus?'El XI síntesis también quedó congelado antes del once oficial. ':'El XI síntesis se añadirá si está disponible antes del once oficial. '}${hasCommunity?'La Comunidad usada es final; no se usa una foto provisional.':'La Comunidad no se añade hasta disponer de consenso final.'} No se reconstruyen escenarios a posteriori.</p>`;
}
