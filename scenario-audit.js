...
function capture(source='save'){
  ...
  const scenarios=scenarioMap(state);
...
function titleFor(id){return id==='user'?'Tú':id==='project'?'Proyecto':id==='strong'?'Señales fuertes':id==='community'?'Comunidad':id}
function aggregate(){
  const audited=records().filter(r=>r.scores),core=audited.filter(r=>['user','project','strong'].every(k=>Number.isFinite(r.scores?.[k]))),keys=['user','project','strong'];
  const rows=keys.map(id=>{const vals=core.map(r=>r.scores[id]);return {id,title:titleFor(id),n:vals.length,avg:vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null,wins:core.filter(r=>{const max=Math.max(r.scores.user,r.scores.project,r.scores.strong);return r.scores[id]===max}).length}}).sort((a,b)=>(b.avg??-1)-(a.avg??-1)||b.wins-a.wins||a.title.localeCompare(b.title,'es'));
...
function scenarioCard(id,rec){const item=id==='user'?rec.user:rec.scenarios?.[id],score=rec.scores?.[id];if(!item)return `<div class="sa-score unavailable"><span>${esc(titleFor(id))}</span><b>—</b><small>No congelado</small></div>`;return `<div class="sa-score ${Number.isFinite(score)?'scored':'pending'}"><span>${esc(titleFor(id))}</span><b>${Number.isFinite(score)?`${score}/11`:'✓'}</b><small>${Number.isFinite(score)?'aciertos':id==='community'?'consenso final congelado':'snapshot congelado'}</small></div>`}
function renderPrediction(){
  const root=ensurePrediction();if(!root)return;const rec=current();
  if(!rec){root.innerHTML='<div class="sa-empty"><b>LIGA DE ESCENARIOS</b><span>Guarda tu XI y congelaremos Proyecto + Señales fuertes para compararlos después con el once oficial.</span></div>';return}
  const settled=Boolean(rec.scores),hasCommunity=Boolean(rec.scenarios?.community);
  root.innerHTML=`<div class="sa-head"><div><span>LIGA DE ESCENARIOS</span><h3>${settled?'Qué escenario leyó mejor el XI oficial':'Escenarios congelados antes del XI oficial'}</h3><p>${settled?'Misma regla 0–11 para todos: cuenta si el jugador fue titular, independientemente de la posición.':'Tu XI, Proyecto y Señales fuertes quedan ligados al guardado. Comunidad solo entra si el consenso ya era final antes del XI oficial.'}</p></div><b>${settled?`Ganador: ${esc(winnerText(rec))}`:`${Object.keys(rec.scenarios||{}).length+1} congelados`}</b></div><div class="sa-scores">${['user','project','strong','community'].map(id=>scenarioCard(id,rec)).join('')}</div><p class="sa-note">${rec.snapshotLate?'Primera captura creada después de un guardado previo, pero todavía antes del XI oficial. ':''}${hasCommunity?'La Comunidad usada es final; no se usa una foto provisional.':'La Comunidad no se añade hasta disponer de consenso final.'} No se reconstruyen escenarios a posteriori.</p>`;
}
function renderSeason(){
  ...
  root.innerHTML=`<div class="sa-season-head"><div><span>LIGA HISTÓRICA DE ESCENARIOS</span><h3>Tú vs Proyecto vs Señales fuertes</h3><p>La clasificación usa solo jornadas donde los tres tienen snapshot previo y resultado. Comunidad aparece aparte porque puede no estar disponible en todas las jornadas.</p></div><b>${a.n} jornada${a.n===1?'':'s'} común${a.n===1?'':'es'}</b></div><div class="sa-leaderboard">${a.rows.map((r,i)=>`<div><em>${i+1}</em><span>${esc(r.title)}</span><b>${fmt(r.avg)}</b><small>${r.wins} victoria${r.wins===1?'':'s'} / empate en cabeza</small></div>`).join('')}</div><div class="sa-community"><span>Comunidad</span><b>${fmt(a.communityAvg)}</b><small>${a.communityN} jornada${a.communityN===1?'':'s'} con consenso final disponible</small></div><div class="sa-history">${history.map(r=>`<div><span>${esc(r.label||label(r.matchId))}</span><b>Tú ${Number.isFinite(r.scores.user)?r.scores.user:'—'}</b><b>Proyecto ${Number.isFinite(r.scores.project)?r.scores.project:'—'}</b><b>Señales ${Number.isFinite(r.scores.strong)?r.scores.strong:'—'}</b><small>${esc(winnerText(r))}</small></div>`).join('')}</div><p class="sa-note">No hay backfill inventado: una jornada solo entra si el escenario fue congelado antes de conocer el XI oficial.</p>`;
}
...
function onSaved(){setTimeout(()=>{capture('save');captureFinalCommunity();settle();render()},180)}
function bootstrap(){const m=match(),mine=saved(),db=read();if(m&&mine?.xi&&!db[m.id]&&!official())setTimeout(()=>{capture('late');captureFinalCommunity();render()},220);else{captureFinalCommunity();settle();render()}}
function install(){
  if(installed)return;if(!window.RMScenarioLab||typeof showSection!=='function'){if(++attempts<100)setTimeout(install,100);return}
  installed=true;bootstrap();
  document.addEventListener('rm-local-prediction-updated',onSaved);
  document.addEventListener('rm-scenario-lab-rendered',()=>setTimeout(()=>{captureFinalCommunity();settle();render()},70));
  document.addEventListener('rm-community-updated',()=>setTimeout(()=>{captureFinalCommunity();settle();render()},90));
  ...
  document.addEventListener('visibilitychange',()=>{if(!document.hidden){captureFinalCommunity();settle();render()}});
  window.RMScenarioAudit=Object.freeze({render,capture,captureFinalCommunity,settle,records,aggregate,current});
}
