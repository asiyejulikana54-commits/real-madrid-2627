...
function loadNavigationHistory(){
  if(!document.querySelector('script[data-navigation-history]')){const script=document.createElement('script');script.src='navigation-history.js?v=1';script.dataset.navigationHistory='1';document.body.appendChild(script)}
}
...
function loadSimpleMode(){
  if(!document.querySelector('link[data-simple-mode]')){const link=document.createElement('link');link.rel='stylesheet';link.href='simple-mode.css?v=1';link.dataset.simpleMode='1';document.head.appendChild(link)}
  if(!document.querySelector('script[data-simple-mode]')){const script=document.createElement('script');script.src='simple-mode.js?v=1';script.dataset.simpleMode='1';document.body.appendChild(script)}
}
...
function install(){
  ...
  syncStats(true);loadEngagementLoop();loadEngagementRewards();loadQuickPicks();loadFavoriteWatch();loadActivityCenter();loadNavigationHistory();loadDeepLinks();loadSharedEntry();loadMomentumPro();
