...
function communityGuide(){
  ...
  return {xi,label:'XI guía · comunidad',meta:total?`${total} pronóstico${total===1?'':'s'} · XI más votado`:'XI más votado disponible',source:'community'};
}
...
function guide(){return communityGuide()}
...
function diffText(s){if(!s.guide)return 'Sin XI de la comunidad disponible';if(!validXi(s.mine))return 'Completa tu XI para compararlo';if(!s.diff?.count)return '11/11 coinciden con la comunidad';const sameCount=11-s.diff.count,ins=s.diff.playerIn.map(display),outs=s.diff.playerOut.map(display);return `${sameCount}/11 coinciden · ${s.diff.count} diferente${s.diff.count===1?'':'s'}${ins.length?` · tú añades ${ins.join(', ')}`:''}${outs.length?` · comunidad ${outs.join(', ')}`:''}`}
...
function install(){
  if(installed)return;if(!window.RMDecisionBoard||!window.RMPredictionReadiness||!document.getElementById('prediccion')){if(++attempts<140)setTimeout(install,100);return}
