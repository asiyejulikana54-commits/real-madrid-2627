...
function quickRecords(){return safe(()=>window.RMQuickPicks?.records?.(),[])||[]}
function quickStats(){return safe(()=>window.RMQuickPicks?.stats?.(),{perfect:0,correct:0,resolved:0,streak2:0})||{perfect:0,correct:0,resolved:0,streak2:0}}
function quickParticipation(){return quickRecords().filter(r=>Object.keys(r?.picks||{}).length>0).length}
...
function badgeState(){
  const p=profile(),best=Number.isFinite(p.best)?p.best:null,fav=favorites().length,voteCount=voteDays().length,streak=visitStreak(),quick=quickStats(),quickPlayed=quickParticipation();
  return [
    ...
    {id:'quick-first',icon:'3',title:'Decisión rápida',desc:'Participa en tus primeros Quick Picks.',unlocked:quickPlayed>=1},
    {id:'quick-perfect',icon:'3/3',title:'Instinto perfecto',desc:'Consigue un 3/3 en Quick Picks.',unlocked:(quick.perfect||0)>=1},
...
function state(){const badges=badgeState(),unlocked=badges.filter(b=>b.unlocked).length,missionsNow=missions(),done=missionsNow.filter(m=>m.done).length,quick=quickStats();return {badges,unlocked,total:badges.length,missions:missionsNow,missionDone:done,missionTotal:missionsNow.length,visitStreak:visitStreak(),bestVisitStreak:bestVisitStreak(),voteDays:voteDays().length,favorites:favorites().length,quick,quickPlayed:quickParticipation(),profile:profile()}}
