...
function outcomeLabel(outcome){return outcome==='alternative'?'GANA ALTERNATIVA':outcome==='project'?'AGUANTA REFERENCIA':outcome==='both'?'JUEGAN LOS DOS':outcome==='neither'?'ENTRA UN TERCERO':'PENDIENTE'}
...
function renderPrediction(){
  ...
  root.innerHTML=`<div class="da-head"><div><span>AUDITORÍA DE DECISIONES</span><h3>Qué debates anticipamos bien</h3><p>Solo declaramos ganador entre referencia y alternativa cuando exactamente uno de los dos entra en el XI oficial. Si juegan ambos o ninguno, el duelo queda sin resolver.</p></div><b>${s.resolved}/${s.debates} resueltos</b></div><div class="da-kpis"><div><span>Referencia aguanta</span><b>${s.projectWins}</b></div><div class="alt"><span>Alternativa gana</span><b>${s.alternativeWins}</b></div><div><span>Sin resolver</span><b>${s.unresolved}</b></div><div><span>Tus cambios ganan</span><b>${s.userWins}/${s.userResolved}</b></div></div><div class="da-list">${visible.length?visible.map(r=>`<article class="${outcomeClass(r.debateOutcome)}"><div><span>${esc(r.slotLabel)}</span><em>${esc(r.levelLabel)}</em></div><b>${esc(display(r.project))} ↔ ${esc(display(r.alternative))}</b><small>${esc(outcomeLabel(r.debateOutcome))}</small><p>${r.debateOutcome==='alternative'?`${esc(display(r.alternative))} entra y ${esc(display(r.project))} no.`:r.debateOutcome==='project'?`${esc(display(r.project))} entra y la alternativa no.`:r.debateOutcome==='both'?'Los dos aparecen en el XI: no atribuimos el puesto a ninguno.':'Ninguno de los dos aparece en el XI: el puesto lo ocupa otra solución.'}</p></article>`).join(''):'<div class="da-empty"><span>No había duelos alternativos congelados.</span></div>'}</div><p class="da-note">Esto calibra nuestras alertas, no sustituye el 0–11 global del pronóstico. Con más jornadas veremos si “Debate fuerte” realmente anticipa cambios de titularidad.</p>`;
}
function renderSeason(){
  ...
  root.innerHTML=`<div class="da-season-head"><div><span>CALIBRACIÓN DE DECISIONES</span><h3>¿Nuestras alertas de cambio sirven de verdad?</h3><p>Medimos únicamente duelos congelados antes del XI oficial y separamos los casos realmente resolubles.</p></div><b>${a.matches} partido${a.matches===1?'':'s'}</b></div><div class="da-season-kpis"><div><span>Alternativa vence</span><b>${pct(a.altRate)}</b><small>${a.altWins}/${a.resolved} duelos resueltos</small></div><div><span>Referencia resiste</span><b>${a.resolved?pct(a.projectWins/a.resolved):'—'}</b><small>${a.projectWins}/${a.resolved}</small></div><div><span>Debate fuerte acierta cambio</span><b>${pct(a.strongAltRate)}</b><small>${a.strongAltWins}/${a.strongResolved} resueltos</small></div><div><span>Tus cambios vencen</span><b>${pct(a.userRate)}</b><small>${a.userWins}/${a.userResolved} comparables</small></div></div><div class="da-history">${recent.map(r=>`<div><span>${esc(r.label||label(r.matchId))}</span><b>${r.summary.alternativeWins} alt.</b><b>${r.summary.projectWins} referencia</b><small>${r.summary.unresolved} sin resolver</small></div>`).join('')}</div><p class="da-note">Las tasas se calculan solo sobre duelos donde exactamente uno de los dos jugadores fue titular. “Ambos” y “ninguno” se conservan como información, pero no fuerzan un ganador.</p>`;
}
