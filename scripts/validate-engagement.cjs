...
for(const marker of ['RMDeepLinks','rmShareCurrent','playerUrl','compareUrl','rm-deep-link-routed','attempts<100'])if(!deep.includes(marker))failures.push(`deep-links.js: falta ${marker}`);
if(/\bfetch\s*\(/.test(deep))failures.push('deep-links.js no debe hacer llamadas de red propias');
if(/MutationObserver\s*\(/.test(deep))failures.push('deep-links.js no debe usar MutationObserver');
if(!deep.includes("q.get('player')")||!deep.includes("q.get('a')")||!deep.includes("q.get('b')"))failures.push('deep-links.js debe resolver jugador y comparativas compartidas');
...
if(!ctx.includes('loadDeepLinks')||!ctx.includes('deep-links.js?v=1'))failures.push('analysis-context.js no carga los enlaces profundos');
...
if(!sw.includes("'deep-links.js'"))failures.push('PWA no incluye los enlaces profundos');
...
console.log('Engagement audit: OK · marcador + duelo diario + Quick Picks + watchlist + centro de novedades + deep links + rachas + retos + insignias + clasificación + jornada + ligas privadas');