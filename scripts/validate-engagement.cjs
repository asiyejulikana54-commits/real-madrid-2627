...
for(const file of ['engagement-loop.js','engagement-rewards.js','quick-picks.js','favorite-watch.js','community-league.js','community.js','personal-nav.js','analysis-context.js','sw.js']){
  try{new vm.Script(read(file),{filename:file})}catch(error){failures.push(`${file} no compila: ${error.message}`)}
}
const js=read('engagement-loop.js'),css=read('engagement-loop.css'),rewards=read('engagement-rewards.js'),rewardsCss=read('engagement-rewards.css'),quick=read('quick-picks.js'),quickCss=read('quick-picks.css'),watch=read('favorite-watch.js'),watchCss=read('favorite-watch.css'),ctx=read('analysis-context.js'),sw=read('sw.js');
...
console.log('Engagement audit: OK · marcador + duelo diario + Quick Picks + watchlist + rachas + retos + insignias + clasificación + jornada + ligas privadas');