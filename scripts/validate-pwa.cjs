...
const sw=read('sw.js'),index=read('index.html'),pwa=read('pwa.js'),personalNav=read('personal-nav.js'),officialReview=read('official-xi-review.js'),quick=read('quick-picks.js'),activity=read('activity-center.js'),deep=read('deep-links.js'),ctx=read('analysis-context.js'),momentum=read('momentum-pro.js');
...
if(!coreMatch)failures.push('sw.js no define CORE_PATHS');
  ...
  for(const required of ['personal-hub.js','personal-nav.js','personal-home.js','engagement-loop.js','engagement-loop.css','engagement-rewards.js','engagement-rewards.css','quick-picks.js','quick-picks.css','favorite-watch.js','favorite-watch.css','activity-center.js','activity-center.css','deep-links.js','momentum-pro.js','momentum-pro.css','community-league.js','community-league.css','official-xi-review.js','official-xi-review.css','stats-pro.js','compare-pro.js','player-experience.js','manifest.webmanifest','app-icon.svg'])if(!unique.has(required))failures.push(`PWA no incluye ${required}`);
}

if(failures.length){console.error('PWA audit: FAIL');for(const f of failures)console.error(`- ${f}`);process.exit(1)}
console.log(`PWA audit: OK · ${quoted.length} recursos · caché versionada · engagement, Momentum PRO, Quick Picks, watchlist, novedades, deep links y revisión oficial offline incluidos`);