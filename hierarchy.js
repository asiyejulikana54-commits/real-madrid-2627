...
window.hierarchyToCompare=function(pos){const s=hSummary(pos);if(!s.boundary||!s.challenger)return;showSection('comparador');let n=0;const go=()=>{if(window.RMComparePro?.setDuel){window.RMComparePro.setDuel(s.boundary.p.name,s.challenger.p.name);return}if(n++<25){setTimeout(go,80);return}const A=document.getElementById('compareA'),B=document.getElementById('compareB');if(A&&B){A.value=s.boundary.p.name;B.value=s.challenger.p.name;if(typeof window.renderCompare==='function')window.renderCompare()}};go()};
