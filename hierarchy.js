...
function hTier(row,index,known,pos){
  ...
  if(index<spots){
    const gap=challenger?row.score-challenger.score:99;
  ...
  const gap=boundary?boundary.score-row.score:99;
  if(index===spots&&gap<=.45)return {id:'duel',label:'Duelo abierto',className:'duel'};
  if(index<=spots+1||gap<=1.20)return {id:'rotation',label:'Rotación',className:'rotation'};
  return {id:'depth',label:'Fondo de plantilla',className:'depth'};
}
function hPositionData(pos){
  const rows=hCandidates(pos),known=rows.filter(x=>x.score!==null);return rows.map((r,i)=>({...r,tier:hTier(r,i,known,pos)}));
}
function hPositionSummary(pos){
  const known=hKnown(pos),spots=hStarterSpots(pos);if(!known.length)return {pos,gap:null,leader:null,boundary:null,challenger:null,starters:[]};
  const boundary=known[Math.min(spots-1,known.length-1)],challenger=known[spots]||null;
...
function hMultiLeader(){
  const starterZones=[];hAllSummaries().forEach(x=>x.starters.forEach(r=>starterZones.push({pos:x.pos,name:r.p.name,score:r.score})));
  const by={};starterZones.forEach(x=>(by[x.name]??=[]).push(x));
...
function hPositionCard(cfg){
  const rows=hPositionData(cfg.id),summary=hPositionSummary(cfg.id),spots=cfg.slots.length;
  const title=summary.starters.length?summary.starters.map(r=>hEsc(r.p.short||r.p.name)).join(spots>1?' + ':''):'Sin datos';
  const sub=summary.challenger?`${hEsc(summary.boundary.p.short||summary.boundary.p.name)} +${summary.gap.toFixed(2)} sobre ${hEsc(summary.challenger.p.short||summary.challenger.p.name)}`:summary.starters.length?'Sin perseguidor calculable':'Pendiente';
  return `<article class="card h-position-card"><div class="h-position-head"><div><span>${cfg.id}</span><h3>${hEsc(cfg.label)}</h3><small>${spots} plaza${spots===1?'':'s'} de referencia</small></div><div><b>${title}</b><small>${sub}</small></div></div><div class="h-position-list">${rows.length?rows.map((r,i)=>hPlayerRow(r,i)).join(''):'<div class="h-empty">Sin jugadores en esta vista.</div>'}</div></article>`;
}
function hBoard(){
...
function renderHierarchy(){
  ...
  <div class="h-integrity"><b>Cómo leer el mapa</b><span>“Titular claro” exige margen sobre el primer jugador que quedaría fuera y presencia en nuestras referencias de XI. “Ventaja” indica zona titular sin ruptura total. “Duelo abierto” aparece cuando la frontera está muy próxima. “Rotación” y “Fondo” describen la situación actual, no el potencial futuro.</span></div>`;
}
