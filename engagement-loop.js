...
function sourceText(row,name){
  if(!row||!name)return '';
  if(same(name,row.project))return `${row.projectAligned}/${row.availableVoices} señales alineadas con nuestra predicción`;
...
function latestMvpMatch(){
  const data=window.RMSeasonData,matches=data?.matches||[],players=safe(()=>typeof window.players!=='undefined'?window.players:(typeof globalThis.players!=='undefined'?globalThis.players:[]),[])||[];
  for(let i=matches.length-1;i>=0;i--){const m=matches[i],rated=players.some(p=>Number.isFinite(safe(()=>data?.officialRatingEntry?.(m.id,p.name)?.value,null)));if(rated)return m}
  return matches.at?.(-1)||null;
}
function mvpHomeHtml(){
  const m=latestMvpMatch();if(!m)return `<article class="egl-card mvp-home"><span>MVP DE LA JORNADA</span><h3>Vota al mejor jugador de cada partido</h3><p>Cuando haya una jornada con notas, podrás elegir aquí tu MVP y entrar a la votación.</p><button type="button" class="btn" data-egl-go="mvp">Ir a MVP →</button></article>`;
  const picks=read(MVP_KEY,{})||{},pick=picks[m.id]||'',label=m.short||m.label||'último partido';
  return `<article class="egl-card mvp-home"><div class="egl-card-head"><div><span>MVP DE LA JORNADA</span><h3>${pick?`Tu MVP: ${esc(display(pick))}`:`¿Quién fue el MVP de ${esc(label)}?`}</h3></div><em class="${pick?'ready':'warn'}">${pick?'VOTO GUARDADO':'POR VOTAR'}</em></div><p>${pick?'Puedes revisar o cambiar tu elección y consultar cómo va la votación.':'Elige al mejor jugador del partido y entra a ver cómo va la votación de MVP.'}</p><button type="button" class="btn" data-egl-mvp="${esc(m.id)}">${pick?'Ver o cambiar mi MVP':'Votar MVP'} →</button></article>`;
}
function profileHtml(p){
  const score=p.avg===null?'—':p.avg.toFixed(1),best=p.best===null?'—':`${p.best}/11`;
  return `<article class="egl-card profile"><div class="egl-card-head"><div><span>TU MARCADOR</span><h3>${esc(p.level)}</h3></div><strong>${score}<small>media /11</small></strong></div><div class="egl-profile-kpis"><div><b>${best}</b><span>récord</span></div><div><b>${p.streak8}</b><span>racha 8+</span></div></div><p>${p.scored.length?`${p.scored.length} jornada${p.scored.length===1?'':'s'} puntuada${p.scored.length===1?'':'s'} · ${p.scored.reduce((n,r)=>n+r.score,0)} aciertos acumulados.`:'Guarda tu primer XI y empezaremos a construir tu historial.'}</p>${p.scenarioRank?`<div class="egl-scenario-rank"><b>#${p.scenarioRank}/${p.scenarioN}</b><span>en tu liga local de escenarios${Number.isFinite(p.scenarioAvg)?` · ${p.scenarioAvg.toFixed(2)}/11`:''}</span></div>`:''}<button type="button" class="btn" data-egl-go="mi-temporada">Ver mi temporada</button></article>`;
}
function debateHtml(row){
  if(!row)return `<article class="egl-card debate empty"><span>DUELO DEL DÍA</span><h3>No hay un debate fuerte abierto ahora mismo</h3><p>Cuando tu XI, la comunidad o los datos discrepen, aparecerá aquí un duelo de un toque.</p><button type="button" class="btn" data-egl-go="prediccion">Ver nuestra predicción</button></article>`;
...
function bind(root,row){
  ...
  root?.querySelectorAll('[data-egl-mvp]').forEach(b=>b.addEventListener('click',()=>{const id=b.dataset.eglMvp;if(window.RMMvpPro?.openMatch)window.RMMvpPro.openMatch(id);else safe(()=>showSection('mvp'))}));
}
function renderHome(){const root=ensureHome();if(!root)return false;const p=profile(),row=debate();root.innerHTML=`<div class="egl-title"><div><span>VUELVE CADA JORNADA</span><h2>Tu marcador, el duelo del día y lo nuevo</h2></div><small>${esc(dayKey())}</small></div><div class="egl-grid">${profileHtml(p)}${debateHtml(row)}${mvpHomeHtml()}${changesHtml()}</div>`;bind(root,row);return true}
...
function install(){
  ...
  ['rm-personal-home-rendered','rm-decision-board-rendered','rm-community-updated','rm-local-prediction-updated','rm-scenario-audit-updated','rm-season-data-ready','rm-ranking-official-ready','rm-daily-debate-voted','rm-mvp-personal-updated'].forEach(ev=>document.addEventListener(ev,()=>setTimeout(render,60)));
