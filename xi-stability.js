...
function state(){
  const api=window.RMRoundImpact;if(!api?.snapshot||!api?.selectionPressure||!api?.signalFor)return null;
  ...
  const rows=(selection.slots||[]).map(slot=>{
    ...
    const mineName=mine?.[slot.key]||'',changed=Boolean(mineName&&canonical(mineName)!==key);
    return {slot,current,pressure,challenger,reinforced,sig,level,label,detail,mineName,changed};
  });
  const counts=rows.reduce((a,r)=>(a[r.level]=(a[r.level]||0)+1,a),{strong:0,stable:0,watch:0,risk:0});
  return {snap,selection,rows,counts,changed:rows.filter(r=>r.changed).length};
}
function card(row){
  const rival=row.challenger?.p?.name||'',action=rival?`data-xs-duel="${esc(row.current)}|${esc(rival)}"`:`data-xs-player="${esc(row.current)}"`;
  const mine=row.changed?`<div class="xs-mine"><span>TU ELECCIÓN</span><b>${esc(display(row.mineName))}</b></div>`:'';
  const compare=row.changed?`<button type="button" class="xs-compare" data-xs-duel="${esc(row.current)}|${esc(row.mineName)}">Comparar con tu cambio →</button>`:'';
...
function html(s){
  ...
  return `<div class="xs-head"><div><span>MAPA DE ESTABILIDAD DEL XI</span><h3>Dónde está firme nuestra propuesta y dónde hay debate</h3><p>Semáforo de los 11 puestos usando las mismas señales de Power, forma, última nota y competencia directa. No representa una probabilidad de titularidad.</p></div><div class="xs-summary"><b>${open}</b><small>puestos a vigilar</small></div></div><div class="xs-kpis"><div class="strong"><span>Respaldados</span><b>${s.counts.strong}</b></div><div><span>Estables</span><b>${s.counts.stable}</b></div><div class="watch"><span>Vigilar</span><b>${s.counts.watch}</b></div><div class="risk"><span>Debate / presión</span><b>${s.counts.risk}</b></div></div>${s.changed?`<div class="xs-user-note"><b>Tu XI se separa de nuestra propuesta en ${s.changed} puesto${s.changed===1?'':'s'}.</b><span>Pulsa “Comparar con tu cambio” para revisar cada decisión.</span></div>`:''}<div class="xs-grid">${s.rows.map(card).join('')}</div><p class="xs-note">Interpretación: <b>Respaldado</b> exige señal positiva y muestra suficiente; <b>Estable</b> significa que no vemos presión fuerte; <b>Vigilar</b> indica un rival creciendo; <b>Debate fuerte / Bajo presión</b> señala competencia directa o deterioro reciente.</p>`;
}
...
function render(){
  ...
  document.dispatchEvent(new CustomEvent('rm-xi-stability-rendered',{detail:{counts:s.counts,userChanges:s.changed,open:s.rows.filter(r=>r.level==='risk'||r.level==='watch').map(r=>({slot:r.slot.key,player:r.current,status:r.label,challenger:r.challenger?.p?.name||null}))}}));
}
function install(){
  if(installed)return;if(!window.RMRoundImpact||!document.getElementById('prediccion')){if(++attempts<80)setTimeout(install,100);return}
  installed=true;render();
  ['rm-round-impact-rendered','rm-current-match-idea-updated','rm-prediction-analytics-updated','rm-local-prediction-updated','rm-season-data-ready'].forEach(ev=>document.addEventListener(ev,()=>setTimeout(render,40)));
