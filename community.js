...
function perfectBadge(count){const n=Number(count)||0;return n>0?`<span class="perfect-badge" title="${n} pleno${n===1?'':'s'}">★${n>1?`×${n}`:''}</span>`:''}
function renderCommunity(data){
  ...
  board.innerHTML=`<div class="leaderboard-head"><span>#</span><span>Usuario</span><span>Puntos</span><span>Partidos</span></div>${data.leaderboard.map((u,i)=>`<div class="leaderboard-row ${u.participantId===getParticipantId()?'me':''}"><b>${i+1}</b><span class="leaderboard-user">${escapeHtml(u.alias)}${perfectBadge(u.perfect)}${u.participantId===getParticipantId()?' <small>(tú)</small>':''}</span><strong>${Number.isFinite(u.points)?u.points:u.hits}<small>/${u.possible}</small></strong><span>${u.scoredMatches}</span></div>`).join('')}`
}
...
const deferredModules=[
  ...
  {css:'hierarchy.css?v=1',cssKey:'hierarchy',script:'hierarchy.js?v=5',scriptKey:'hierarchy'}
];
...
async function loadCriticalModules(){
  ...
  await loadModule({script:'matchday.js?v=7',scriptKey:'matchday'});
  await loadModule({css:'matchday-pro.css?v=1',cssKey:'matchday-pro',script:'matchday-pro.js?v=3',scriptKey:'matchday-pro'});
