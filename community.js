...
function renderCommunityPitch(popularXI,total){
  ...
  pitch.innerHTML=slots.map(s=>{const choice=popularXI?.[s[0]];return `<div class="community-slot" style="left:${s[2]}%;top:${s[3]}%"><small>${s[1]}</small><b>${choice?escapeHtml(displayName(choice.name)):'—'}</b><span>${choice?`${choice.percentage}%`:'0%'}</span></div>`}).join('')
}
...
function renderCommunity(data){
  ...
  document.getElementById('communityPolls').innerHTML=[['Lateral derecho','rb'],['Lateral izquierdo','lb'],['Central izquierdo','lcb'],['Central derecho','rcb'],['Mediapunta','am'],['Banda derecha','rw']].map(([label,key])=>pollBlock(label,data.slotShares?.[key])).join('');
