(()=>{
const css=document.createElement('link');css.rel='stylesheet';css.href='matchday.css?v=2';css.dataset.matchdayCss='2';document.head.appendChild(css);
const matchSection=['partido','⚽','Partido','Centro del partido','Previa, XI oficial, aciertos y análisis de la jornada.'];
...
let pollCache=window.RMMatchdayPollData||null,communityData=window.RMCommunityData||null,clockTimer=null,installed=false;
...
function ourXi(){return safe(()=>rayoXI,{})||{}}
...
function formatKickoff(){
  ...
  return new Intl.DateTimeFormat('es-ES',{weekday:'short',day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}).format(new Date(raw)).replace(',',' ·').toUpperCase();
}
...
function phase(){
  ...
  if(hasOfficial)return {id:'official',index:2,label:'XI oficial',eyebrow:'XI OFICIAL',title:'Ya conocemos los once titulares',copy:'Compara el once real con nuestra idea y con tu predicción.',action:'Ver mis aciertos',section:'prediccion',clock:Number.isFinite(kickoff)?`Empieza en ${countdown(kickoff)}`:'XI publicado'};
  if(Number.isFinite(deadline)&&now>=deadline)return {id:'closed',index:1,label:'Predicción cerrada',eyebrow:'PREDICCIONES CERRADAS',title:'Ahora toca esperar el XI oficial',copy:'Tu pronóstico ya no se puede modificar. En cuanto publiquemos el once, calcularemos los aciertos.',action:'Ver comunidad',section:'comunidad',clock:Number.isFinite(kickoff)?`Empieza en ${countdown(kickoff)}`:'Cerrado'};
  return {id:'pre',index:0,label:'Previa abierta',eyebrow:'PREVIA',title:`Prepara tu XI contra el ${m.rival}`,copy:'Haz tu predicción, compárala con nuestra idea y sigue los debates antes del cierre.',action:savedPrediction()?'Revisar mi XI':'Hacer mi predicción',section:'prediccion',clock:Number.isFinite(deadline)?`Cierra en ${countdown(deadline)}`:'Predicción abierta'};
}
...
function ourCard(){
  ...
  return `<article class="card md-xi-card"><div class="md-card-head"><span class="md-label">NUESTRA IDEA</span><b>${score===null?'11 jugadores':`${score}/11`}</b></div><h3>XI que estamos valorando</h3>${lineupChips(names)}<button class="btn" data-md-go="once" data-md-preset="rayo">Abrir en Constructor</button></article>`;
}
...
function pollsHtml(){
  if(!backendAvailable()&&!pollCache)return `<div class="md-polls-offline"><b>Encuestas comunitarias no cargadas en esta versión</b><span>El resto del Centro del partido funciona con los datos locales y oficiales disponibles.</span></div>`;
...
function sectionOpened(){loadQuickPolls(false);loadCommunityIfNeeded(false);render()}
function install(){
  ...
  window.RMMatchdayCenter=Object.freeze({render,phase,refresh:sectionOpened});
}
install();
})();
