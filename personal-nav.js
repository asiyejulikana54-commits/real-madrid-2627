...
function install(){
  ...
  installed=true;refresh();loadPredictionAnalytics();scheduleRoundImpact();loadXiStability();loadDecisionBoard();loadDecisionAudit();loadDecisionProfile();loadDecisionCoach();loadDecisionActions();loadPredictionReadiness();loadScenarioLab();loadConsensusXI();loadScenarioAudit();loadOfficialXiReview();loadReviewArchive();loadCommunityLeague();
  const base=showSection;showSection=function(id){if(id==='mi-temporada'){loadPersonalSeasonPro();loadDecisionProfile();loadReviewArchive()}if(id==='prediccion')loadDecisionCoach();if(id==='comunidad'){loadCommunityPro();loadCommunityLeague()}base(id);setTimeout(refresh,0);if(id==='mi-temporada')setTimeout(()=>window.RMPersonalSeasonPro?.render?.(),0);if(id==='comunidad'){setTimeout(()=>window.RMCommunityPro?.render?.(),120);setTimeout(()=>window.RMCommunityLeague?.render?.(),160)}if(id==='prediccion'||id==='mi-temporada')setTimeout(()=>window.RMPredictionAnalytics?.render?.(),140);if(id==='prediccion'||id==='partido')setTimeout(()=>window.RMXIStability?.render?.(),180);if(id==='prediccion'||id==='partido')setTimeout(()=>window.RMDecisionBoard?.render?.(),240);if(id==='prediccion')setTimeout(()=>window.RMDecisionActions?.render?.(),290);if(id==='prediccion'||id==='partido')setTimeout(()=>window.RMPredictionReadiness?.render?.(),320);if(id==='prediccion'||id==='partido')setTimeout(()=>window.RMScenarioLab?.render?.(),345);if(id==='prediccion'||id==='partido')setTimeout(()=>window.RMConsensusXI?.render?.(),370);if(id==='prediccion'||id==='mi-temporada')setTimeout(()=>window.RMDecisionAudit?.render?.(),410);if(id==='prediccion'||id==='mi-temporada')setTimeout(()=>window.RMScenarioAudit?.render?.(),455);if(id==='prediccion'||id==='mi-temporada')setTimeout(()=>window.RMDecisionProfile?.render?.(),520);if(id==='prediccion')setTimeout(()=>window.RMDecisionCoach?.render?.(),545);if(id==='prediccion'||id==='partido')setTimeout(()=>window.RMOfficialXIReview?.render?.(),580);if(id==='mi-temporada')setTimeout(()=>window.RMReviewArchive?.render?.(),620)};
  const active=document.querySelector('.section.active')?.id;if(active==='mi-temporada'){loadPersonalSeasonPro();loadDecisionProfile();loadReviewArchive()}if(active==='prediccion')loadDecisionCoach();if(active==='comunidad'){loadCommunityPro();loadCommunityLeague()}
