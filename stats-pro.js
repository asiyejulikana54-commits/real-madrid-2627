...
function playerWindow(name){
  ...
  const sample=Math.min(totalMinutes,450)/450,efficiency=totalPoints?ratedMinutes/totalPoints:null,power=Number.isFinite(rating)?rating*(.75+.25*sample):null;
...
function methodHtml(){return `<section class="sp-method"><div><span>VENTANA DINÁMICA</span><h3>El periodo sí recalcula las métricas</h3><p>Competición y últimos 1/3/5 partidos no son un filtro visual: vuelven a calcular media ponderada por minutos, aporte, Min/punto, Power RM, forma, estabilidad y percentiles con esa ventana.</p></div><div><span>RESPALDO</span><p><b>Muestra</b> = minutos jugados hasta 450. <b>Min/punto</b> usa solo minutos con nota oficial; los minutos 3×SC quedan fuera del denominador. <b>Cobertura</b> = apariciones con nota / apariciones jugadas. <b>Participación</b> = partidos con minutos / partidos de la ventana. SC no equivale a 0 y nunca se convierte en una nota ficticia.</p></div></section>`}
...
function render(){
  ...
  root.innerHTML=`${leadersHtml(rows)}${controlsHtml()}${insight(rows)}${compareHtml(rows)}${methodHtml()}<div class="sp-table"><div class="sp-head"><span>#</span><span>Jugador</span><span>Media</span><span>Power</span><span>Forma</span><span>Min</span><span>Min/punto</span><span>Señal grupo</span><span>Respaldo</span><span>Serie</span></div><div class="sp-body">${rows.length?rows.map(rowHtml).join(''):'<div class="sp-empty"><b>No hay resultados</b><span>Cambia los filtros para volver a ver jugadores.</span></div>'}</div></div><p class="sp-note">Power RM se recalcula con la ventana elegida y ajusta la media por muestra hasta 450 minutos. En Min/punto, menor es mejor y solo cuentan minutos con nota oficial. Los minutos 3×SC se muestran en la muestra, pero no penalizan la eficiencia. Percentil, muestra, cobertura, participación y estabilidad son lecturas separadas.</p>`;
