...
function scenarioTitle(id){return id==='user'?'Tú':id==='project'?'Referencia':id==='strong'?'Señales fuertes':id==='consensus'?'XI síntesis':id==='community'?'Comunidad':id}
...
function resultLabel(r){return r.result==='win'?'Tú ganas':r.result==='loss'?'Referencia gana':'Empate'}
...
function decisionText(r){
  ...
  return `<div class="ra-mini-kpis"><div><span>Tus cambios</span><b>${uw}/${user}</b><small>ganan entre resolubles</small></div><div><span>Alternativas</span><b>${alt}/${res}</b><small>vencen a la referencia</small></div><div><span>Debates fuertes</span><b>${Number(d.strongAlternativeWins)||0}/${Number(d.strongResolved)||0}</b><small>cambio confirmado</small></div></div>${r.decisionStrict?'':'<p class="ra-warning">La auditoría de decisiones de esta jornada fue capturada tarde: se conserva, pero no debe usarse para calibrar tasas.</p>'}`
}
...
function roundCard(r){
  const community=finite(r.communityScore)?` · Comunidad ${r.communityScore}/11`:'',exact=finite(r.exactUser)&&finite(r.exactProject)&&finite(r.exactPossible)?`<div class="ra-exact"><span>Coincidencia por rol</span><b>Tú ${r.exactUser}/${r.exactPossible} · Referencia ${r.exactProject}/${r.exactPossible}${finite(r.exactCommunity)?` · Comunidad ${r.exactCommunity}/${r.exactPossible}`:''}</b></div>`:'';
  return `<details class="ra-round ${r.result}"><summary><div><span>${esc(r.label)}</span><small>${r.scoreStrict?'AUDITORÍA ESTRICTA':'SNAPSHOT TARDÍO'}</small></div><strong>Tú ${r.userScore}/11 · Referencia ${r.projectScore}/11${community}</strong><em>${esc(resultLabel(r))}</em></summary><div class="ra-detail">${exact}<div class="ra-diff-grid"><section><span>TÚ VISTE Y REFERENCIA NO</span>${chips(r.youSaw,'Ningún acierto diferencial tuyo.')}</section><section><span>REFERENCIA VIO Y TÚ NO</span>${chips(r.projectSaw,'Ningún acierto diferencial de la referencia.')}</section></div><section class="ra-block"><span>ESCENARIOS</span>${scenarioText(r)}</section><section class="ra-block"><span>DECISIONES</span>${decisionText(r)}</section>${r.scoreStrict?'':'<p class="ra-warning">Esta jornada se muestra por transparencia, pero queda fuera de los KPIs del archivo porque la foto principal no fue estrictamente previa al XI oficial.</p>'}</div></details>`;
}
...
function render(){
  ...
  root.innerHTML=`<div class="ra-head"><div><span>ARCHIVO DE REVISIONES OFICIALES</span><h3>Cada jornada, qué acertaste tú y qué acertó la referencia</h3><p>Une el 0–11, los diferenciales reales, la Liga de escenarios y la Auditoría de decisiones. Los KPIs usan solo snapshots estrictos previos al XI oficial.</p></div><b>${s.strict.length}<small> jornadas estrictas</small></b></div><div class="ra-kpis"><div><span>Balance vs Referencia</span><b>${s.wins}-${s.draws}-${s.losses}</b><small>victorias · empates · derrotas</small></div><div><span>Tu media</span><b>${fmt(s.userAvg)}</b><small>Referencia ${fmt(s.projectAvg)}</small></div><div><span>Ventaja media</span><b>${s.edge===null?'—':`${s.edge>0?'+':''}${s.edge.toFixed(2)}`}</b><small>aciertos por jornada</small></div><div><span>Mejor jornada</span><b>${esc(best)}</b><small>${s.late?`${s.late} registro${s.late===1?'':'s'} tardío${s.late===1?'':'s'} fuera de KPI`:'sin snapshots tardíos'}</small></div></div><div class="ra-filters"><button type="button" data-ra-filter="all" class="${activeFilter==='all'?'active':''}">Todas <b>${s.rows.length}</b></button><button type="button" data-ra-filter="win" class="${activeFilter==='win'?'active':''}">Tú ganas <b>${s.rows.filter(r=>r.result==='win').length}</b></button><button type="button" data-ra-filter="draw" class="${activeFilter==='draw'?'active':''}">Empates <b>${s.rows.filter(r=>r.result==='draw').length}</b></button><button type="button" data-ra-filter="loss" class="${activeFilter==='loss'?'active':''}">Referencia <b>${s.rows.filter(r=>r.result==='loss').length}</b></button></div><div class="ra-list">${visible.length?visible.map(roundCard).join(''):'<div class="ra-empty"><span>No hay jornadas en este filtro.</span></div>'}</div><p class="ra-note">No reconstruimos jornadas ni completamos retrospectivamente XIs que no fueron congelados. Los registros tardíos pueden consultarse, pero no contaminan los KPIs estrictos.</p>`;
