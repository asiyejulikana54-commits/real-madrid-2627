...
function roundCard(r){
  ...
  return `<details class="ra-round ${r.result}"><summary><div><span>${esc(r.label)}</span><small>${r.scoreStrict?'AUDITORÍA ESTRICTA':'SNAPSHOT TARDÍO'}</small></div><strong>Tú ${r.userScore}/11 · Referencia ${r.projectScore}/11${community}</strong><em>${esc(resultLabel(r))}</em></summary><div class="ra-detail">${exact}<div class="ra-diff-grid"><section><span>TÚ VISTE Y REFERENCIA NO</span>${chips(r.youSaw,'Ningún acierto diferencial tuyo.')}</section><section><span>REFERENCIA VIO Y TÚ NO</span>${chips(r.projectSaw,'Ningún acierto diferencial del proyecto.')}</section></div><section class="ra-block"><span>ESCENARIOS</span>${scenarioText(r)}</section><section class="ra-block"><span>DECISIONES</span>${decisionText(r)}</section>${r.scoreStrict?'':'<p class="ra-warning">Esta jornada se muestra por transparencia, pero queda fuera de los KPIs del archivo porque la foto principal no fue estrictamente previa al XI oficial.</p>'}</div></details>`;
}
