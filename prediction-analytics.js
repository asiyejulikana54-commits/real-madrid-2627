...
function triWinner(rows){
  ...
  const values=[['Tú',avg(rows,'userScore')],['Referencia',avg(rows,'projectScore')],['Comunidad',avg(rows,'communityScore')]],best=Math.max(...values.map(([,v])=>v)),leaders=values.filter(([,v])=>v===best).map(([name])=>name);
...
function roundWinner(rec){
  const candidates=[['Tú',rec?.userScore],['Referencia',rec?.projectScore],['Comunidad',rec?.communityScore]].filter(([,v])=>Number.isFinite(v));if(candidates.length<2)return 'Esperando resultados';
...
function renderPrediction(){
  ...
  root.innerHTML=`<div class="pa-head"><div><span>MEDIDOR DE ACIERTO</span><h3>Tú · Referencia · Comunidad</h3><p>${scored?'Tres referencias con la misma regla: 1 punto por cada titular acertado.':'Tu XI y la referencia están congelados; la comunidad se fijará con su consenso final al cierre.'}</p></div><b>${scored?esc(roundWinner(rec)):'Pendiente'}</b></div><div class="pa-score-grid"><article><span>TU XI</span><strong>${scored?rec.userScore:'—'}<small>/11</small></strong><small>${scored?'titulares acertados':'guardado'}</small></article><article><span>REFERENCIA</span><strong>${scored?rec.projectScore:'—'}<small>/11</small></strong><small>${scored?'titulares acertados':'referencia congelada'}</small></article><article><span>COMUNIDAD</span><strong>${communityScored?rec.communityScore:'—'}<small>/11</small></strong><small>${communityScored?`${rec.communityTotal||0} pronósticos`:esc(communityStatus(rec))}</small></article></div>${scored?`<div class="pa-round"><span>RESULTADO DE LA JORNADA</span><b>${esc(roundWinner(rec))}</b><small>Tú vs Referencia: ${rec.edge>0?`+${rec.edge}`:rec.edge}</small></div>`:''}${Number.isFinite(rec.exactUser)&&Number.isFinite(rec.exactProject)?`<div class="pa-exact"><span>COINCIDENCIA POR ROL</span><b>Tú ${rec.exactUser}/${rec.exactPossible} · Referencia ${rec.exactProject}/${rec.exactPossible}${Number.isFinite(rec.exactCommunity)?` · Comunidad ${rec.exactCommunity}/${rec.exactPossible}`:''}</b><small>DFC y MC equivalentes no penalizan por invertir izquierda/derecha.</small></div>`:''}${late?'<p class="pa-note">La referencia se capturó al activar esta función porque tu XI ya estaba guardado; desde el próximo guardado la foto será exacta al momento del pronóstico.</p>':'<p class="pa-note">Tu XI y la referencia se congelan al guardar. La comunidad se congela al cierre, cuando ya no admite más votos. Así el consenso final no puede cambiar después de conocer el once oficial.</p>'}`;
}
...
function leaderboard(tri){
  ...
  const rows=[{name:'Tú',avg:tri.userAvg},{name:'Referencia',avg:tri.projectAvg},{name:'Comunidad',avg:tri.communityAvg}].sort((a,b)=>b.avg-a.avg||a.name.localeCompare(b.name,'es'));
...
function renderSeason(){
  ...
  root.innerHTML=`<div class="pa-season-head"><div><span>TÚ · REFERENCIA · COMUNIDAD</span><h3>¿Quién predice mejor el XI?</h3><p>La clasificación usa solo jornadas en las que existen resultados para los tres, para que las medias sean comparables.</p></div><b>${tri.n?esc(tri.winner.text):'Sin muestra común'}</b></div><div class="pa-season-kpis"><div><span>Tu media</span><b>${tri.userAvg===null?'—':tri.userAvg.toFixed(2)}</b><small>/11 · muestra común</small></div><div><span>Referencia</span><b>${tri.projectAvg===null?'—':tri.projectAvg.toFixed(2)}</b><small>/11 · muestra común</small></div><div><span>Comunidad</span><b>${tri.communityAvg===null?'—':tri.communityAvg.toFixed(2)}</b><small>/11 · consenso final</small></div><div><span>Jornadas comunes</span><b>${tri.n}</b><small>${pending} pendientes de XI oficial</small></div></div>${leaderboard(tri)}${recent.length?`<div class="pa-history">${recent.map(r=>`<div><span>${esc(r.label||label(r.matchId))}</span><b>Tú ${r.userScore}/11</b><b>Referencia ${r.projectScore}/11</b><b>Comunidad ${Number.isFinite(r.communityScore)?`${r.communityScore}/11`:'—'}</b><em class="${roundWinner(r).includes('Tú')?'win':roundWinner(r).includes('Referencia')&&!roundWinner(r).includes('Empate')?'loss':'draw'}">${esc(roundWinner(r))}</em></div>`).join('')}</div>`:'<div class="pa-empty">Cuando tengamos el primer XI oficial cerrado aparecerá aquí la comparación jornada a jornada.</div>'}${s.n>tri.n?`<p class="pa-note">Hay ${s.n-tri.n} jornada${s.n-tri.n===1?'':'s'} anterior${s.n-tri.n===1?'':'es'} con Tú vs Referencia pero sin consenso comunitario congelado; no entra${s.n-tri.n===1?'':'n'} en la clasificación de tres.</p>`:''}`;
}
