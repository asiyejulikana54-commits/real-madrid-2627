...
function scoreWinner(rec){
  const rows=[['Tú',rec?.userScore],['Referencia',rec?.projectScore],['Comunidad',rec?.communityScore]].filter(([,v])=>Number.isFinite(v));
...
function scenarioWinner(rec){
  ...
  const labels={user:'Tú',project:'Referencia',strong:'Señales fuertes',consensus:'XI síntesis',community:'Comunidad'};
...
function exactLine(rec){
  ...
  return `<div class="oxr-exact"><span>COINCIDENCIA POR ROL</span><b>Tú ${rec.exactUser}/${rec.exactPossible} · Referencia ${rec.exactProject}/${rec.exactPossible}${Number.isFinite(rec.exactCommunity)?` · Comunidad ${rec.exactCommunity}/${rec.exactPossible}`:''}</b><small>Los dos DFC y los dos MC son intercambiables; invertir su orden no resta.</small></div>`;
}
function renderPrediction(){
  ...
  const edge=rec.userScore-rec.projectScore,edgeText=edge>0?`Tú +${edge}`:edge<0?`Referencia +${Math.abs(edge)}`:'Empate';
  root.innerHTML=`<div class="oxr-head"><div><span>REVISIÓN DEL XI OFICIAL</span><h3>Qué acertaste tú y qué acertó la referencia</h3><p>La lectura usa exclusivamente las fotos congeladas antes del XI oficial. La puntuación principal mide presencia entre los once titulares; la lectura por rol agrupa puestos equivalentes como DFC y MC.</p></div><b>${esc(scoreWinner(rec))}</b></div><div class="oxr-score-grid">${scoreCard('TU XI',rec.userScore,edgeText,'you')}${scoreCard('REFERENCIA',rec.projectScore,edgeText==='Empate'?'Mismo resultado':edge>0?'Por detrás de ti':'Por delante','project')}${scoreCard('COMUNIDAD',rec.communityScore,Number.isFinite(rec.communityScore)?`${rec.communityTotal||0} pronósticos`:'Sin consenso final','community')}</div>${exactLine(rec)}<div class="oxr-diff-grid"><article><span>TÚ VISTE Y LA REFERENCIA NO</span>${chips(d.youSaw,'good')}<small>${d.youSaw?.length?'Fueron titulares y solo estaban en tu XI congelado.':'No hubo un titular exclusivo de tu predicción frente a la referencia.'}</small></article><article><span>LA REFERENCIA VIO Y TÚ NO</span>${chips(d.projectSaw,'warn')}<small>${d.projectSaw?.length?'Fueron titulares y solo estaban en la propuesta congelada.':'No hubo un titular exclusivo de la referencia frente a ti.'}</small></article></div>${decision?`<div class="oxr-audit"><div><span>DECISIONES CONGELADAS</span><b>${decision.resolved}/${decision.debates} debates resolubles</b></div><div class="oxr-audit-grid"><p><strong>${decision.alternativeWins}</strong><small>alternativas ganan</small></p><p><strong>${decision.projectWins}</strong><small>aguanta referencia</small></p><p><strong>${decision.userWins}/${decision.userResolved}</strong><small>tus cambios ganan</small></p><p><strong>${decision.strongAlternativeWins}/${decision.strongResolved}</strong><small>señales fuertes cambian titular</small></p></div></div>`:''}${scenario?`<div class="oxr-scenario"><div><span>LIGA DE ESCENARIOS</span><b>${esc(scenario.winners.map(r=>r.label).join(' + '))} · ${scenario.best}/11</b></div><div>${scenario.rows.map((r,i)=>`<p class="${i===0?'leader':''}"><span>${esc(r.label)}</span><b>${r.score}/11</b></p>`).join('')}</div></div>`:''}<div class="oxr-actions"><button class="btn" data-oxr-go="mi-temporada">Ver historial</button><button class="btn" data-oxr-go="partido">Abrir centro del partido</button></div><p class="oxr-note">No se reconstruyen predicciones ni escenarios después de conocer el once. Si no había snapshot previo, esa comparación queda fuera de la auditoría.</p>`;
...
function renderMatchday(){
  ...
  root.innerHTML=`<div class="oxr-compact-head"><div><span>REVISIÓN RÁPIDA</span><h3>XI oficial vs pronósticos congelados</h3></div><button class="btn" data-oxr-open="1">Ver detalle</button></div><div class="oxr-compact-grid"><p><span>Tú</span><b>${rec.userScore}/11</b></p><p><span>Referencia</span><b>${rec.projectScore}/11</b></p><p><span>Comunidad</span><b>${Number.isFinite(rec.communityScore)?`${rec.communityScore}/11`:'—'}</b></p><p><span>Mejor escenario</span><b>${scenario?`${esc(lead)} · ${scenario.best}/11`:'—'}</b></p></div>`;
