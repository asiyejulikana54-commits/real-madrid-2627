...
function sourceList(ctx){
  ...
  const community=ctx?.scenarios?.find(s=>s.id==='community');if(community?.valid&&validXi(community.xi))out.push({id:'community',label:ctx.community?.final?'Comunidad':'Comunidad · provisional',xi:cloneXi(community.xi),kind:'community'});
  return out;
}
...
function solve(rows){
  ...
  let best=null,bestScore=-1,nodes=0;const used=new Set(),picked={};
  function dfs(i,score){
    if(++nodes>80000)return;if(score+suffix[i]<bestScore)return;
    if(i===order.length){if(score>bestScore){bestScore=score;best={...picked}}return}
    const row=order[i];for(const c of row.list){if(used.has(c.key))continue;used.add(c.key);picked[row.slot.key]=c;dfs(i+1,score+c.votes*100+c.tie);delete picked[row.slot.key];used.delete(c.key)}
  }
  dfs(0,0);
  if(best)return {picked:best,nodes,exact:nodes<=80000};
  const fallback={};used.clear();for(const row of rows){const c=row.list.find(x=>!used.has(x.key));if(!c)return null;fallback[row.slot.key]=c;used.add(c.key)}return {picked:fallback,nodes,exact:false};
}
...
function state(){
  const ctx=lab();if(!ctx)return null;const sources=sourceList(ctx);if(sources.length<2)return null;const rows=candidateRows(sources),solution=solve(rows);if(!solution)return null;
  ...
  return {ctx,sources,total,xi,slots:slotsState,unanimous:slotsState.filter(s=>s.unanimous).length,majority:slotsState.filter(s=>!s.unanimous&&s.majority).length,split:slotsState.filter(s=>s.split).length,dProject,dMine,isClosed:closed(),sameMine:validXi(mine)&&defs().every(d=>same(mine[d.key],xi[d.key])),exact:solution.exact};
}
...
function html(s){const src=s.sources.map(x=>x.label).join(' · '),projectChange=s.dProject.in.length;return `<div class="cx-head"><div><span>XI SÍNTESIS</span><h3>Qué once sale al cruzar todas las perspectivas disponibles</h3><p>No es una probabilidad ni mezcla fuentes independientes: votan los escenarios disponibles y los empates conservan estabilidad favoreciendo Proyecto.</p></div><div class="cx-summary"><b>${s.unanimous} unánimes</b><span>${s.majority} mayoría · ${s.split} divididos</span></div></div><div class="cx-sources"><span>${esc(src)}</span><b>${s.total} perspectivas</b></div><div class="cx-grid">${s.slots.map(x=>slotHtml(x,s.total)).join('')}</div><div class="cx-foot"><p>${projectChange?`La síntesis cambia ${projectChange} titular${projectChange===1?'':'es'} respecto a Proyecto.`:'La síntesis mantiene los mismos 11 titulares que Proyecto.'}${!s.exact?' La resolución usó el modo seguro de respaldo.':''}</p><button type="button" data-cx-load ${s.isClosed||s.sameMine?'disabled':''}>${s.isClosed?'Predicción cerrada':s.sameMine?'Ya está en tu borrador':'Probar XI síntesis'}</button></div>`}
function render(){const s=state();if(!s)return;const root=ensurePrediction();if(root){root.innerHTML=html(s);root.querySelector('[data-cx-load]')?.addEventListener('click',load)}const compact=ensureMatchday();if(compact){compact.innerHTML=`<div><span>XI SÍNTESIS</span><b>${s.unanimous} puestos unánimes · ${s.split} divididos</b><small>${esc(s.sources.map(x=>x.label).join(' · '))}</small></div><button type="button" class="btn" data-cx-open>Ver síntesis</button>`;compact.querySelector('[data-cx-open]')?.addEventListener('click',()=>{safe(()=>showSection('prediccion'));setTimeout(()=>document.getElementById('consensusXI')?.scrollIntoView?.({behavior:'smooth',block:'start'}),80)})}document.dispatchEvent(new CustomEvent('rm-consensus-xi-rendered',{detail:{sources:s.sources.map(x=>x.id),unanimous:s.unanimous,majority:s.majority,split:s.split,projectChanges:s.dProject.in.length}}))}
