...
function sourceList(ctx,{finalCommunityOnly=false}={}){
  ...
  const reference=out.find(s=>s.id==='project')?.xi||null;
  return reference&&window.RMLineupSemantics?out.map(source=>({...source,xi:window.RMLineupSemantics.normalizeAgainst(reference,source.xi)})):out;
}
...
function diff(base,xi){
  if(window.RMLineupSemantics){const c=window.RMLineupSemantics.compare(base,xi);return {in:c.playerIn,out:c.playerOut,changes:c.changes,count:c.count,roleMatches:c.roleMatches,possible:c.possible}}
  const A=new Map(Object.values(base||{}).filter(Boolean).map(n=>[canonical(n),n])),B=new Map(Object.values(xi||{}).filter(Boolean).map(n=>[canonical(n),n]));return {in:[...B].filter(([k])=>!A.has(k)).map(([,n])=>n),out:[...A].filter(([k])=>!B.has(k)).map(([,n])=>n),count:[...B].filter(([k])=>!A.has(k)).length}
}
function buildState({finalCommunityOnly=false}={}){
  ...
  const project=sources.find(s=>s.id==='project')?.xi||{},mine=current(),dProject=diff(project,xi),dMine=validXi(mine)?diff(xi,mine):{in:[],out:[],count:0};
  const sameMine=validXi(mine)&&(window.RMLineupSemantics?window.RMLineupSemantics.equivalent(xi,mine):defs().every(d=>same(mine[d.key],xi[d.key])));
  return {ctx,sources,total,xi,slots:slotsState,unanimous:slotsState.filter(s=>s.unanimous).length,majority:slotsState.filter(s=>!s.unanimous&&s.majority).length,split:slotsState.filter(s=>s.split).length,dProject,dMine,isClosed:closed(),sameMine,exact:solution.exact,solveMode:solution.mode,finalCommunityOnly};
}
...
function state(){
  ...
  const provisionalDiff=provisionalCommunity&&audit?diff(audit.xi,live.xi):{in:[],out:[],count:0};
  return {...live,provisionalCommunity,auditXi:audit?.xi||null,auditSources:audit?.sources||[],auditTotal:audit?.total||0,provisionalChanges:provisionalDiff.count||0,auditReady:Boolean(audit)};
}
...
function auditNotice(s){
  if(s.provisionalCommunity){
    const movement=s.provisionalChanges?` La comunidad provisional mueve ${s.provisionalChanges} cambio${s.provisionalChanges===1?'':'s'} real${s.provisionalChanges===1?'':'es'} frente a la versión auditable.`:' La versión auditable mantiene los mismos jugadores en sus roles.';
  ...
  return `<div class="cx-audit-note safe"><b>AUDITABLE</b><span>Esta síntesis puede congelarse sin incorporar una Comunidad provisional. Los dos DFC y los dos MC se tratan como grupos intercambiables: invertir su orden no crea un debate falso.</span></div>`;
}
function html(s){const src=s.sources.map(x=>x.label).join(' · '),projectChange=s.dProject.count??s.dProject.in.length,mode=s.solveMode==='exact'?'resolución exacta':s.solveMode==='bounded'?'resolución acotada':'modo seguro de respaldo';return `<div class="cx-head"><div><span>XI SÍNTESIS</span><h3>Qué once sale al cruzar todas las perspectivas disponibles</h3><p>No es una probabilidad ni mezcla fuentes independientes: votan los escenarios disponibles. Antes de votar normalizamos los puestos equivalentes para que Rüdiger–Huijsen y Huijsen–Rüdiger sean el mismo dúo de centrales.</p></div><div class="cx-summary"><b>${s.unanimous} unánimes</b><span>${s.majority} mayoría · ${s.split} divididos</span></div></div>${auditNotice(s)}<div class="cx-sources"><span>${esc(src)}</span><b>${s.total} perspectivas · ${esc(mode)}</b></div><div class="cx-grid">${s.slots.map(x=>slotHtml(x,s.total)).join('')}</div><div class="cx-foot"><p>${projectChange?`La síntesis introduce ${projectChange} cambio${projectChange===1?'':'s'} real${projectChange===1?'':'es'} respecto a Proyecto.`:'La síntesis mantiene los mismos jugadores en sus roles que Proyecto.'}</p><button type="button" data-cx-load ${s.isClosed||s.sameMine?'disabled':''}>${s.isClosed?'Predicción cerrada':s.sameMine?'Ya está en tu borrador':'Probar XI síntesis'}</button></div>`}
function render(){const s=state();if(!s)return;const root=ensurePrediction();if(root){root.innerHTML=html(s);root.querySelector('[data-cx-load]')?.addEventListener('click',load)}const compact=ensureMatchday();if(compact){compact.innerHTML=`<div><span>XI SÍNTESIS${s.provisionalCommunity?' · EXPLORATORIO':''}</span><b>${s.unanimous} puestos unánimes · ${s.split} divididos</b><small>${esc(s.sources.map(x=>x.label).join(' · '))}${s.provisionalCommunity?` · histórico: ${s.auditTotal} fuentes auditables`:''}</small></div><button type="button" class="btn" data-cx-open>Ver síntesis</button>`;compact.querySelector('[data-cx-open]')?.addEventListener('click',()=>{safe(()=>showSection('prediccion'));setTimeout(()=>document.getElementById('consensusXI')?.scrollIntoView?.({behavior:'smooth',block:'start'}),80)})}document.dispatchEvent(new CustomEvent('rm-consensus-xi-rendered',{detail:{sources:s.sources.map(x=>x.id),auditableSources:s.auditSources.map(x=>x.id),provisionalCommunity:s.provisionalCommunity,provisionalChanges:s.provisionalChanges,unanimous:s.unanimous,majority:s.majority,split:s.split,projectChanges:s.dProject.count??s.dProject.in.length}}))}
function install(){if(installed)return;if(!window.RMScenarioLab||!window.RMLineupSemantics||typeof currentPredictionXI!=='function'){if(++attempts<100)setTimeout(install,100);return}installed=true;render();['rm-scenario-lab-rendered','rm-lineup-semantics-ready','rm-community-updated','rm-local-prediction-updated','rm-prediction-draft-updated','rm-decision-board-rendered'].forEach(ev=>document.addEventListener(ev,()=>setTimeout(render,70)));document.getElementById('prediccion')?.addEventListener('change',e=>{if(e.target?.matches?.('[id^="pred_"]'))setTimeout(render,0)});window.RMConsensusXI=Object.freeze({render,state,auditableState,load})}
