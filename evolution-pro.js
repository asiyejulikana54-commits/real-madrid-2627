...
function matches(){return analysis()?.matches?.()||allMatches()}
function contextLabel(){return analysis()?.label?.()||`${matches().length} jornadas`}
...
function jornadaIndex(matchId){return allMatches().findIndex(m=>m.id===matchId)}
...
function leaderCards(rows){
  ...
  const rise=[...jornada].filter(x=>x.pair.delta>.049).sort((a,b)=>b.pair.delta-a.pair.delta)[0]||null;
  const fall=[...jornada].filter(x=>x.pair.delta<-.049).sort((a,b)=>a.pair.delta-b.pair.delta)[0]||null;
  const progress=rows.map(x=>({stat:x,pair:seasonDelta(x)})).filter(x=>x.pair&&x.pair.delta>.049).sort((a,b)=>b.pair.delta-a.pair.delta)[0]||null;
  const streak=[...rows].filter(x=>x.streak>0).sort((a,b)=>b.streak-a.streak||(b.recentAvg??-99)-(a.recentAvg??-99))[0]||null;
...
function journeySnapshot(rows,index){
  ...
  const prev=index>0?rows.map(stat=>({stat,pair:strictPair(stat,index-1,index)})).filter(x=>x.pair):[];
  const mover=prev.length?[...prev].sort((a,b)=>Math.abs(b.pair.delta)-Math.abs(a.pair.delta))[0]:null;
...
function journeyOverview(rows){
  ...
  return `<section class="ep-journeys"><div class="ep-journeys-head"><div><div class="eyebrow">MAPA DE JORNADAS</div><h3>La ventana en el orden real</h3><p>Cada tarjeta conserva la cronología oficial del proyecto dentro del contexto compartido. La media es la media simple de las notas oficiales publicadas.</p></div><span class="pill">${snaps.length} jornadas</span></div><div class="ep-journey-strip">${snaps.map(s=>{const leader=s.leaders[0],teamDelta=s.index>0&&snaps[s.index-1]?.average!==null&&s.average!==null?s.average-snaps[s.index-1].average:null;return `<article class="ep-journey-card ${s.index===snaps.length-1?'current':''}"><div class="ep-journey-id"><b>J${jornadaIndex(s.match.id)+1}</b><span>${esc(s.match.short||s.match.label)}</span></div><h4>${esc(s.match.label)}</h4><div class="ep-journey-main"><span><small>Media notas</small><b>${s.average===null?'—':s.average.toFixed(2)}</b></span><span class="${trendClass(teamDelta)}"><small>Vs anterior</small><b>${s.index===0?'Inicio':signed(teamDelta,2)}</b></span></div><button type="button" ${leader?`data-ep-journey-player="${esc(leader.stat.player.name)}"`:''} ${leader?'':'disabled'}><small>Mejor nota</small><b>${leader?`${esc(display(leader.stat.player))} · ${leader.row.value.toFixed(2)}`:'—'}</b>${s.leaders.length>1?`<em>Empate ×${s.leaders.length}</em>`:''}</button><div class="ep-journey-foot"><span>${s.rated.length} con nota</span><span>${s.mover&&s.index>0?`${s.mover.pair.delta>.049?'↑':s.mover.pair.delta<-.049?'↓':'→'} ${esc(display(s.mover.stat.player))} ${signed(s.mover.pair.delta,1)}`:'—'}</span></div></article>`}).join('')}</div></section>`;
}
function latestComparison(rows){
  ...
  return `<section class="card ep-jornada"><div class="ep-jornada-head"><div><div class="eyebrow">CAMBIO DE JORNADA · ESTRICTO</div><h3>${esc(jornadaLabel(prev))} → ${esc(jornadaLabel(latest))}</h3><p>Solo entran jugadores con nota oficial en ambas jornadas consecutivas de la ventana. No se busca una nota anterior si faltó una de las dos.</p></div><span class="pill">${comp.length} comparables</span></div><div class="ep-jornada-list">${comp.length?comp.map(x=>`<button data-ep-jornada-player="${esc(x.stat.player.name)}"><span><b>${esc(display(x.stat.player))}</b><small>${x.stat.player.pos}</small></span><i>${x.pair.prev.value.toFixed(1)}</i><em>→</em><i>${x.pair.latest.value.toFixed(1)}</i><strong class="${trendClass(x.pair.delta)}">${signed(x.pair.delta,2)}</strong></button>`).join(''):'<div class="ep-empty">No hay jugadores con nota oficial en las dos últimas jornadas.</div>'}</div></section>`
}
...
function contextControls(){const c=context();return `<div class="card ep-explorer"><div class="ep-controls"><span class="pill">CONTEXTO · ${esc(contextLabel())}</span><select id="epCompetition" class="select">${competitionOptions()}</select><select id="epHorizon" class="select"><option value="ALL" ${c.horizon==='ALL'?'selected':''}>Todo el periodo</option><option value="5" ${c.horizon==='5'?'selected':''}>Últimos 5 partidos</option><option value="3" ${c.horizon==='3'?'selected':''}>Últimos 3 partidos</option><option value="1" ${c.horizon==='1'?'selected':''}>Último partido</option></select></div></div>`}
function rowHtml(s){
...
function sortName(){return state.sort==='recent'?'Forma reciente':state.sort==='season'?'Media de ventana':state.sort==='streak'?'Racha 7+':'Momentum'}
...
function bodyHtml(){
  ...
  return `<div class="ep-shell"><div class="ep-title"><div><div class="eyebrow">EVOLUCIÓN PRO</div><h2>Quién sube, quién cae y cómo cambia cada jornada</h2><p>Forma, Momentum, rachas y cambios recalculados en <b>${esc(contextLabel())}</b>.</p></div><span class="pill">${matches().length} jornadas</span></div>${contextControls()}${leaderCards(all)}${journeyOverview(all)}${latestComparison(all)}<div class="card ep-explorer"><div class="ep-controls"><div class="ep-pos">${positionButtons()}</div><input id="epSearch" class="input" value="${esc(state.query)}" placeholder="Buscar jugador…"><select id="epMin" class="select"><option value="1" ${state.minRated===1?'selected':''}>1+ notas</option><option value="2" ${state.minRated===2?'selected':''}>2+ notas</option><option value="3" ${state.minRated===3?'selected':''}>3+ notas</option></select><select id="epSort" class="select"><option value="momentum" ${state.sort==='momentum'?'selected':''}>Ordenar: Momentum</option><option value="recent" ${state.sort==='recent'?'selected':''}>Ordenar: Forma</option><option value="season" ${state.sort==='season'?'selected':''}>Ordenar: Media</option><option value="streak" ${state.sort==='streak'?'selected':''}>Ordenar: Racha 7+</option></select></div><div class="ep-reading"><b>LECTURA ACTUAL</b><span>${esc(summaryText(rows))}</span><small>Momentum usa las últimas notas disponibles dentro del contexto: últimas 2 vs 2 anteriores; con menos muestra, última vs anterior. “Mayor subida”, “Mayor bajada” y “Cambio de jornada” exigen dos jornadas consecutivas; subida o bajada solo se etiqueta cuando el cambio supera 0,05.</small></div><div class="ep-list">${rows.length?rows.map(rowHtml).join(''):'<div class="ep-empty">No hay jugadores que cumplan estos filtros.</div>'}</div></div><div class="ep-integrity"><b>Cronología</b><span>${chronology?.corrected?esc(chronology.note):'Las jornadas se leen en el orden definido por RMSeasonData.'} <strong>SC no entra como 0</strong> y una ausencia tampoco. Inicio→fin solo aparece cuando el jugador tiene nota oficial en ambos extremos de la ventana.</span></div></div>`;
}
function bind(root){
  ...
  const comp=root.querySelector('#epCompetition');if(comp)comp.onchange=e=>analysis()?.set?.({competition:e.target.value},'evolution');
  const horizon=root.querySelector('#epHorizon');if(horizon)horizon.onchange=e=>analysis()?.set?.({horizon:e.target.value},'evolution');
...
function install(){
  ...
  ['rm-season-data-ready','rm-season-order-corrected','rm-ranking-official-ready','rm-modules-ready','rm-analysis-context-updated'].forEach(name=>document.addEventListener(name,()=>setTimeout(mount,0)));
}
window.RMEvolutionPro=Object.freeze({render:mount,state:()=>({...state}),jornadaLabel,seasonDelta,context});
