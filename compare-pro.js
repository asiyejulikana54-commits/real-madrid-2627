...
function commonSample(a,b){
  const rows=seasonMatches().map((match,index)=>{
    ...
    const comparable=av!==null&&bv!==null,c=comparable?cmp(av,bv,true,.05):0;
    return {match,index,av,bv,comparable,winner:comparable?c:null,delta:comparable?av-bv:null};
  });
  ...
  const aAvg=avg(shared.map(x=>x.av)),bAvg=avg(shared.map(x=>x.bv)),delta=Number.isFinite(aAvg)&&Number.isFinite(bAvg)?aAvg-bAvg:null;
  const winner=cmp(aAvg,bAvg,true,.05);
  return {rows,shared,aw,bw,ties,aAvg,bAvg,delta,winner,coverage:rows.length?shared.length/rows.length:0};
}
...
function commonReading(common,global,a,b){
  ...
  if(common.shared.length===1)return {tone:'limited',title:'Muestra común muy corta',copy:`Solo comparten una jornada con nota oficial. ${common.aw>common.bw?an:common.bw>common.aw?bn:'Ninguno'} sale por delante en ese partido, pero no es suficiente para una conclusión estable.`};
  if(gw&&cw&&gw!==cw)return {tone:'split',title:'Lectura dividida',copy:`La comparación global favorece a ${gw>0?an:bn}, pero en las jornadas con nota para ambos la media favorece a ${cw>0?an:bn}. Conviene mirar la muestra antes de decidir.`};
  if(gw&&cw===gw)return {tone:'aligned',title:'La muestra común confirma el global',copy:`La ventaja global de ${gw>0?an:bn} también aparece cuando comparamos únicamente las jornadas en las que ambos tienen nota oficial.`};
  if(!gw&&cw)return {tone:'shared',title:'El global empata, la muestra común separa',copy:`Las métricas globales están equilibradas, pero en partidos compartidos ${cw>0?an:bn} obtiene mejor media.`};
  return {tone:'balanced',title:'Duelo equilibrado también en muestra común',copy:'Ni el agregado global ni los partidos compartidos ofrecen una ventaja clara con la tolerancia actual.'};
}
function commonHtml(common,global,a,b){
  const reading=commonReading(common,global,a,b),total=common.rows.length,an=display(a.p.name),bn=display(b.p.name);
  return `<section class="cp-common"><div class="cp-common-head"><div><span>MUESTRA COMÚN · ESTRICTA</span><h3>${common.shared.length}/${total} jornadas comparables</h3><p>Solo entra una jornada si ambos tienen nota oficial en ese mismo partido. SC, ausencia o partido sin nota quedan como — y nunca como 0.</p></div><div class="cp-common-score"><small>H2H por jornada</small><b>${common.aw}<i>–</i>${common.bw}</b><em>${common.ties} empate${common.ties===1?'':'s'}</em></div></div><div class="cp-common-kpis"><div><span>${esc(an)}</span><b>${fmt(common.aAvg,2)}</b><small>Media compartida</small></div><div><span>Diferencia</span><b class="${common.delta>0.049?'a':common.delta<-.049?'b':'tie'}">${Number.isFinite(common.delta)?`${common.delta>0?'+':''}${common.delta.toFixed(2)}`:'—'}</b><small>${common.shared.length} partidos comunes</small></div><div><span>${esc(bn)}</span><b>${fmt(common.bAvg,2)}</b><small>Media compartida</small></div></div><div class="cp-common-reading ${reading.tone}"><span>${esc(reading.title)}</span><p>${esc(reading.copy)}</p></div><div class="cp-common-grid">${common.rows.map(x=>`<div class="cp-common-row ${x.comparable?'':'missing'}"><span>${esc(jornadaLabel(x.match,x.index))}</span><b class="${x.winner>0?'winner':''}">${x.av===null?'—':x.av.toFixed(2)}</b><i>${x.comparable?(x.winner>0?'←':x.winner<0?'→':'='):'·'}</i><b class="${x.winner<0?'winner':''}">${x.bv===null?'—':x.bv.toFixed(2)}</b></div>`).join('')}</div></section>`;
}
