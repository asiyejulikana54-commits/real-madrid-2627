...
function commonSample(a,b){
  const rows=seasonMatches().map((match,index)=>{
    ...
    const am=av!==null?matchMinutes(match.id,a.p?.name):0,bm=bv!==null?matchMinutes(match.id,b.p?.name):0,comparable=av!==null&&bv!==null&&am>0&&bm>0,sharedWeight=comparable?Math.min(am,bm):0,c=comparable?cmp(av,bv,true,.05):0;
...
function optionExists(select,value){return [...select.options].some(o=>o.value===value)}
function syncDistinctOptions(A,B){[...A.options].forEach(o=>o.disabled=o.value===B.value);[...B.options].forEach(o=>o.disabled=o.value===A.value)}
function setDuel(a,b,update=true){
  const A=document.getElementById('compareA'),B=document.getElementById('compareB');if(!A||!B)return;
  if(optionExists(A,a))A.value=a;
  if(optionExists(B,b)&&canonical(b)!==canonical(A.value))B.value=b;
  if(canonical(A.value)===canonical(B.value)){const alt=[...B.options].find(o=>canonical(o.value)!==canonical(A.value));if(alt)B.value=alt.value}
  syncDistinctOptions(A,B);renderPro();if(update)syncUrl(A.value,B.value);
}
...
function install(){
  ...
  installed=true;const base=renderCompare;renderCompare=function(){renderPro()};
  syncDistinctOptions(A,B);
  A.onchange=()=>{syncDistinctOptions(A,B);renderPro();syncUrl(A.value,B.value)};
  B.onchange=()=>{syncDistinctOptions(A,B);renderPro();syncUrl(A.value,B.value)};
  applyUrl();syncDistinctOptions(A,B);renderPro();
