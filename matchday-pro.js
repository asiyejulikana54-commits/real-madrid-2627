...
function comparisonHtml(){
  ...
  if(!hasUser)return `<section class="mdp-compare empty"><div><span>TU XI VS REFERENCIA</span><h3>Guarda una predicción para comparar</h3><p>Cuando tengas 11 jugadores distintos veremos coincidencias y diferencias por puesto.</p></div><button class="btn primary" type="button" data-mdp-go="prediccion">Crear mi XI</button></section>`;
  return `<section class="mdp-compare"><div class="mdp-compare-head"><div><span>TU XI VS REFERENCIA</span><h3>${overlap}/11 jugadores coinciden</h3><p>${diffs.length?`${diffs.length} puesto${diffs.length===1?'':'s'} con una elección distinta.`:'Coincidimos también en la colocación de los 11.'}</p></div><b>${diffs.length}<small>cambios</small></b></div>${diffs.length?`<div class="mdp-diff-list">${diffs.map(d=>`<div><span>${esc(d.label)}</span><b>${esc(display(d.a||'—'))}</b><i>→</i><strong>${esc(display(d.b||'—'))}</strong></div>`).join('')}</div>`:'<div class="mdp-perfect">✓ Mismo XI y mismas posiciones</div>'}</section>`;
}
function watchHtml(){
  const rows=watchItems(),hasUser=validXI(saved()?.xi||{});
  return `<section class="mdp-watch"><div class="mdp-block-title"><span>QUÉ MIRAR</span><h3>Dudas que pueden definir el once</h3><p>${hasUser?'Primero aparecen los debates donde tu XI y la referencia discrepan.':'Las principales dudas que ya estamos siguiendo para esta jornada.'}</p></div><div class="mdp-watch-grid">${rows.map(({d,a,b,same})=>`<article class="${same?'same':''}"><span>${esc(d.label)}</span><h4>${d.options.map(display).map(esc).join(' / ')}</h4><div><small>Referencia</small><b>${esc(a.join(' · ')||'Ninguno')}</b></div><div><small>Tu XI</small><b>${esc(hasUser?(b.join(' · ')||'Ninguno'):'Pendiente')}</b></div></article>`).join('')}</div></section>`;
}
function renderTop(){
